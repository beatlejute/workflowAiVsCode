...
- **Фактическая модель kilo-агента в pipeline-view.** Роутеры `kilo/kilo-auto/free` и `kilo/openrouter/free` выбирают модель сами. Раннер пишет в лог строку `AGENT_MODELS agent="…"`, пока агент работает (при смене набора моделей) и после выхода. Текущая и завершённые стадии показывают агента с моделью: `kilo-free(dots-3-note-preview)`, `openrouter-free(nemotron, ling)`; у агента со своей моделью — как раньше, `gpt-luna`. Со старым раннером строки нет, и виден id агента из START.
