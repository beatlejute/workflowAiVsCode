...
`
  ...
  suite('KanbanTicketTreeItem - Recurring Badges', () => {
    test('creates tree item with sync icon for recurring ticket', () => {
      const recurringDefinitions: import('../../data/types').RecurringDefinition[] = [
        {
          id: 'rec-001',
          name: 'Weekly Review',
          enabled: true,
      ...
      const ticket: Ticket = {
        ...
        updated_at: '2026-03-05T00:00:00Z',
        completed_at: '',
        recurring_source: 'rec-001'
      };

      const item = new KanbanTicketTreeItem(ticket, tempWorkflowRoot, recurringDefinitions);

      assert.ok(item.iconPath, 'Should have iconPath');
      assert.strictEqual((item.iconPath as vscode.ThemeIcon).id, 'sync', 'Icon should be sync for recurring ticket');
    });

    test('creates tree item without sync icon for non-recurring ticket', () => {
      const ticket: Ticket = {
        id: 'KANBAN-005',
      ...
      assert.ok(item.iconPath, 'Should have iconPath');
      assert.strictEqual((item.iconPath as vscode.ThemeIcon).id, 'circle-filled', 'Icon should be priority circle for regular ticket');
    });

    test('tooltip contains recurring definition info', () => {
      const recurringDefinitions: import('../../data/types').RecurringDefinition[] = [
        {
          id: 'rec-002',
          name: 'Daily Standup',
      ...
      const ticket: Ticket = {
        ...
        completed_at: '',
        recurring_source: 'rec-002'
      };

      const item = new KanbanTicketTreeItem(ticket, tempWorkflowRoot, recurringDefinitions);

      assert.ok(item.tooltip, 'Should have tooltip');
      const tooltipMd = item.tooltip as vscode.MarkdownString;
      assert.ok(tooltipMd.value.includes('Daily Standup'), 'Tooltip should contain definition name');
      assert.ok(tooltipMd.value.includes('cron'), 'Tooltip should contain trigger type');
      assert.ok(tooltipMd.value.includes('0 9 * * 1-5'), 'Tooltip should contain cron expression');
      assert.ok(tooltipMd.value.includes('10'), 'Tooltip should contain instance count');
    });

    test('tooltip does not contain recurring info when no definitions provided', () => {
      const ticket: Ticket = {
        id: 'REC-003',
        title: 'Orphan Recurring Ticket',
        ...
        updated_at: '2026-03-05T00:00:00Z',
        completed_at: '',
        recurring_source: 'non-existent-def'
      };
      ...
      assert.ok(item.tooltip, 'Should have tooltip');
      const tooltipMd = item.tooltip as vscode.MarkdownString;
      assert.ok(!tooltipMd.value.includes('**Recurring**:'), 'Tooltip should not contain recurring info section when definitions empty');
    });
  });
