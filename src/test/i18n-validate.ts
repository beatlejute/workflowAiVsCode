 ...
 * Can be run with: node --loader ts-node/esm src/test/i18n-validate.ts
 * Or simply: npx ts-node src/test/i18n-validate.ts
 */
...
function validateI18n(): ValidationResult {
  ...
  const missingPackageNlsKeys: string[] = [];
  ...
  const missingBundleKeys: string[] = [];
  ...
  const extraPackageNlsKeys: string[] = [];
