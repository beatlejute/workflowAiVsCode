...
var WorkflowStore = class _WorkflowStore {
  ...
  getTicketsByStatus(status) {
    return Array.from(this.tickets.values()).filter((t2) => t2.status === status);
  }
  /**
   * Get tickets filtered by priority
   */
  getTicketsByPriority(priority) {
    return Array.from(this.tickets.values()).filter((t2) => t2.priority === priority);
  }
  /**
  ...
  getTicketsWithDependency(dependencyId) {
    return Array.from(this.tickets.values()).filter(
      (t2) => t2.dependencies.includes(dependencyId)
    );
  }
  /**
   * Get all plans
...
function getL10nDir() {
  ...
  if (!extensionPath) {
    return path3.join(__dirname, "..", "l10n");
  }
  return path3.join(extensionPath, "l10n");
}
function loadBundle(locale) {
  ...
  if (locale === "auto" || locale === "en") {
    bundlePath = path3.join(l10nDir, "bundle.l10n.json");
  } else {
    const localeMap = {
    ...
    if (!bundleFile) {
      ...
      bundlePath = path3.join(l10nDir, bundleFile);
    }
  }
  try {
    const content = fs3.readFileSync(bundlePath, "utf-8");
...
function getCurrentLocale() {
  ...
  const locale = config.get("locale", "auto");
  return locale;
}
function t(key, ...args) {
  const locale = getCurrentLocale();
  if (locale === "auto") {
    const message2 = vscode.l10n.t(key);
    return formatMessage(message2, args);
  }
...
function onLocaleChanged() {
  return vscode.workspace.onDidChangeConfiguration((e) => {
    if (e.affectsConfiguration("workflow.locale")) {
      ...
      const locale = getCurrentLocale();
      const message = locale === "auto" ? t("Using VS Code locale") : t("Using locale: {0}", locale);
...
function extractPlanId(parentPlan) {
  ...
  const basename4 = parentPlan.replace(/\\/g, "/").split("/").pop() || parentPlan;
  return basename4.replace(/\.md$/, "");
}
function getReviewCacheKey(reviews) {
  if (!reviews || reviews.length === 0) {
    return "empty";
  }
  return `${reviews.length}:${reviews.map((r) => r.status).join(",")}`;
}
var reviewBadgesCache = /* @__PURE__ */ new Map();
var cacheHits = 0;
var cacheMisses = 0;
function getReviewBadges(reviews) {
...
// src/ui/sidebar-tree-provider.ts
var sidebarTicketsCache = /* @__PURE__ */ new Map();
var sidebarTreeItemCache = /* @__PURE__ */ new Map();
var sidebarPerfMetrics = {
  cacheHits: 0,
  cacheMisses: 0,
  treeItemCacheHits: 0,
  treeItemCacheMisses: 0
};
var SidebarTreeItem = class extends vscode2.TreeItem {
...
var TicketTreeItem = class extends SidebarTreeItem {
  constructor(ticket, workflowRoot) {
    ...
    super(label, vscode2.TreeItemCollapsibleState.None, "ticket", ticket.id);
    this.ticket = ticket;
    ...
    this.command = {
      command: "vscode.open",
      title: t("Open Ticket"),
      arguments: [vscode2.Uri.file(getTicketPath(ticket, workflowRoot))]
    };
  }
};
function getOrCreateSidebarTreeItem(ticket, workflowRoot) {
...
var StatusGroupTreeItem = class extends SidebarTreeItem {
  constructor(status, count) {
    ...
    super(label, vscode2.TreeItemCollapsibleState.Expanded, "status-group", status);
    this.status = status;
    this.count = count;
    this.contextValue = "status-group";
  }
};
var FilterInfoTreeItem = class extends SidebarTreeItem {
  constructor(planId, planTitle) {
    const label = `\u{1F50D} ${planId}: ${planTitle}`;
    super(label, vscode2.TreeItemCollapsibleState.None, "filter-info", `filter-${planId}`);
    this.planId = planId;
    this.planTitle = planTitle;
    this.description = "";
    this.tooltip = `Active filter: ${planId}
Click to clear filter`;
    this.iconPath = new vscode2.ThemeIcon("filter", new vscode2.ThemeColor("charts.orange"));
...
var PlanTreeItem = class extends SidebarTreeItem {
  constructor(plan, workflowRoot, isCurrent, isDecomposing = false) {
    const label = plan.id;
    const description = plan.title;
    const groupId = isCurrent ? "current" : "archive";
    super(label, vscode2.TreeItemCollapsibleState.None, "plan", plan.id);
    this.plan = plan;
    this.description = description;
    this.tooltip = `${plan.id}: ${plan.title}
${t("Status")}: ${plan.status}`;
    this.iconPath = isDecomposing ? new vscode2.ThemeIcon("loading~spin") : new vscode2.ThemeIcon("notebook");
    this.contextValue = isCurrent ? "plan-current" : "plan-archive";
    const planPath = path4.join(
      workflowRoot,
      "plans",
      groupId,
      `${plan.id}.md`
    ...
    this.command = {
      command: "vscode.open",
      title: t("Open Plan"),
      arguments: [vscode2.Uri.file(planPath)]
    };
  }
};
var PlanGroupTreeItem = class extends SidebarTreeItem {
  constructor(groupType, count) {
    ...
    const state = groupType === "current" ? vscode2.TreeItemCollapsibleState.Expanded : vscode2.TreeItemCollapsibleState.Collapsed;
...
var ReportTreeItem = class extends SidebarTreeItem {
  constructor(report, workflowRoot) {
    ...
    super(label, vscode2.TreeItemCollapsibleState.None, "report", report.id);
    this.report = report;
    this.description = description;
    this.tooltip = `${report.id}: ${report.title}
${t("Type")}: ${report.type}
${t("Created")}: ${report.created_at}`;
    this.iconPath = new vscode2.ThemeIcon("document");
    ...
    this.command = {
      command: "vscode.open",
      title: t("Open Report"),
      arguments: [vscode2.Uri.file(reportPath)]
    };
  }
};
...
var TicketsTreeProvider = class {
  ...
  getTicketsForStatus(status) {
    ...
    if (!sidebarTicketsCache.has(cacheKey)) {
      sidebarTicketsCache.set(cacheKey, /* @__PURE__ */ new Map());
    }
...
var PlansTreeProvider = class {
  ...
  onDidChangeTreeData = this._onDidChangeTreeData.event;
  workflowRoot = null;
  decomposingPlanIds = /* @__PURE__ */ new Set();
  /**
   * Mark a plan as decomposing (shows spinner icon)
   */
  setDecomposing(planId) {
...
var SkillTreeItem = class extends SidebarTreeItem {
  constructor(skillId, skillPath, stagesCount, isUnlinked) {
    ...
    super(label, vscode2.TreeItemCollapsibleState.None, "skill", skillId);
...
Stages: ${stagesCount}
Path: ${skillPath}`;
    this.iconPath = isUnlinked ? new vscode2.ThemeIcon("warning", new vscode2.ThemeColor("notificationsWarningIcon.foreground")) : new vscode2.ThemeIcon("symbol-method");
    this.command = {
      command: "vscode.open",
      title: t("Open Skill"),
      arguments: [vscode2.Uri.file(skillPath)]
    };
  }
};
var SkillsTreeProvider = class {
  ...
  setWorkflowRoot(root) {
    ...
    const skillsPattern = path4.join(root, "src", "skills", "*", "SKILL.md");
    this.skillsWatcher = vscode2.workspace.createFileSystemWatcher(skillsPattern);
   ...
   */
  async getSkills() {
    const skillsDir = path4.join(this.workflowRoot, "src", "skills");
    try {
      const entries = await vscode2.workspace.fs.readDirectory(vscode2.Uri.file(skillsDir));
      const items = [];
      const skillDirs = entries.filter(([_, type2]) => type2 === vscode2.FileType.Directory).map(([name]) => name).sort();
      const pipeline = this.store.getPipeline();
      const stageSkills = this.getStageSkillsFromPipeline(pipeline);
      for (const skillDir of skillDirs) {
        const skillPath = path4.join(skillsDir, skillDir, "SKILL.md");
        const skillUri = vscode2.Uri.file(skillPath);
        try {
          await vscode2.workspace.fs.stat(skillUri);
        } catch {
          continue;
        }
...
var LogFileTreeItem = class extends SidebarTreeItem {
  constructor(fileName, filePath, modifiedDate) {
    ...
    super(label, vscode2.TreeItemCollapsibleState.None, "log", fileName);
...
Modified: ${modifiedDate.toLocaleString()}`;
    this.iconPath = new vscode2.ThemeIcon("output");
    this.command = {
      command: "vscode.open",
      title: t("Open Log"),
      arguments: [vscode2.Uri.file(filePath)]
    };
  }
};
var LogsTreeProvider = class {
  ...
  setWorkflowRoot(root) {
    ...
    const logsPattern = path4.join(root, "logs", "*");
    this.logsWatcher = vscode2.workspace.createFileSystemWatcher(logsPattern);
   ...
   */
  async getLogFiles() {
    const logsDir = path4.join(this.workflowRoot, "logs");
    try {
      const entries = await vscode2.workspace.fs.readDirectory(vscode2.Uri.file(logsDir));
      const items = [];
      for (const [fileName, fileType] of entries) {
        if (fileType === vscode2.FileType.File) {
          const filePath = path4.join(logsDir, fileName);
          try {
            const stats = await vscode2.workspace.fs.stat(vscode2.Uri.file(filePath));
...
var sortedTicketsCache = /* @__PURE__ */ new Map();
var treeItemCache = /* @__PURE__ */ new Map();
var perfMetrics = {
  cacheHits: 0,
  cacheMisses: 0,
  treeItemCacheHits: 0,
...
var KanbanTicketTreeItem = class extends vscode3.TreeItem {
  constructor(ticket, workflowRoot) {
    const label = ticket.id;
    const reviewBadges = getReviewBadges(ticket.reviews);
    const description = reviewBadges ? `${reviewBadges} ${ticket.title}` : ticket.title;
    ...
    this.command = {
      command: "vscode.open",
      title: t("Open Ticket"),
      arguments: [vscode3.Uri.file(ticketPath)]
    };
  }
};
function getOrCreateTreeItem(ticket, workflowRoot) {
  const cacheKey = `${ticket.id}:${ticket.updated_at}`;
...
function createTicketTooltip(ticket) {
  ...
  const deps = ticket.dependencies.length > 0 ? ticket.dependencies.join(", ") : t("None");
...
function getTicketIcon2(priority) {
  if (priority <= 1) {
    return new vscode3.ThemeIcon("circle-filled", new vscode3.ThemeColor("notificationsErrorIcon.foreground"));
  } else if (priority === 2) {
    return new vscode3.ThemeIcon("circle-filled", new vscode3.ThemeColor("notificationsWarningIcon.foreground"));
  } else if (priority === 3) {
    return new vscode3.ThemeIcon("circle-filled", new vscode3.ThemeColor("notificationsInfoIcon.foreground"));
  } else {
    return new vscode3.ThemeIcon("circle-filled", new vscode3.ThemeColor("terminal.ansiGreen"));
  }
}
...
function createKanbanProviders(store) {
  const done = new KanbanTreeProvider(store, "done" /* Done */);
    ...
    try {
      ...
      await vscode4.window.withProgress(
        ...
        async (progress, token) => {
          ...
          this.progressCancellationToken = new vscode4.CancellationTokenSource();
          token.onCancellationRequested(() => {
...
// src/ui/pipeline-tree-provider.ts
var PipelineTreeItem = class extends vscode5.TreeItem {
  constructor(label, collapsibleState, itemType, id) {
    super(label, collapsibleState);
    this.itemType = itemType;
    this.id = id;
...
var CurrentStageTreeItem = class extends PipelineTreeItem {
  constructor(stage, agent, fallbackAgent, skill, ticket, attempt, maxAttempts) {
    ...
    this.iconPath = new vscode5.ThemeIcon("gear~spin");
...
var CompletedStageTreeItem = class _CompletedStageTreeItem extends PipelineTreeItem {
  constructor(stage, elapsed, success, ticket, agent, skill, statusChange, outputLines, reportInfo, logLineHint, logFile) {
    const icon = success ? "\u2705" : "\u274C";
...
var StatisticsTreeItem = class extends PipelineTreeItem {
  constructor(stagesStarted, retries, gotos) {
    ...
    this.gotos = gotos;
    this.iconPath = new vscode5.ThemeIcon("graph");
...
var HistoryTreeItem = class extends PipelineTreeItem {
  constructor(history) {
    ...
    this.iconPath = new vscode5.ThemeIcon("history");
...
var HistoryItemTreeItem = class extends PipelineTreeItem {
  constructor(entry) {
    ...
    super(
      label,
      hasReports ? vscode5.TreeItemCollapsibleState.Collapsed : vscode5.TreeItemCollapsibleState.None,
      "history-item",
...
var HistoryReportTreeItem = class extends PipelineTreeItem {
  constructor(reportId, reportPath, runNumber) {
    const label = `\u{1F4C4} ${reportId}`;
    super(
      label,
      vscode5.TreeItemCollapsibleState.None,
      "history-report",
    ...
    this.command = {
      command: "vscode.open",
      title: "Open Report",
      arguments: [vscode5.Uri.file(reportPath)]
    };
  }
};
...
function getPipelineStateIcon(state) {
  switch (state) {
    case "idle" /* Idle */:
      return new vscode5.ThemeIcon("circle-outline");
    case "running" /* Running */:
      return new vscode5.ThemeIcon("loading~spin");
    case "error" /* Error */:
      return new vscode5.ThemeIcon("error", new vscode5.ThemeColor("notificationsErrorIcon.foreground"));
    case "completed" /* Completed */:
      return new vscode5.ThemeIcon("check", new vscode5.ThemeColor("terminal.ansiGreen"));
    default:
      return new vscode5.ThemeIcon("circle-outline");
  }
}
...
function createCompletedStageTooltip(stage, elapsed, success, ticket, agent, skill, statusChange, outputLines) {
  const markdown = new vscode5.MarkdownString();
  markdown.isTrusted = true;
...
function createHistoryReportTooltip(reportId, reportPath) {
  const markdown = new vscode5.MarkdownString();
  markdown.isTrusted = true;
...
var PipelineTreeProvider = class {
  // Track report info for current stage
  ...
  currentOutputLines = [];
  // Track output lines for current stage
  completedStages = [];
  stageOccurrences = /* @__PURE__ */ new Map();
  // Track occurrence index per stage name
  currentRunReports = [];
  // Track reports for current run
  runStartTime = 0;
  // Timestamp when current run started
  currentRunLogFile;
  // Track log file for current run
  currentRunPlanId;
  // Track plan ID for current run
  currentStageReport;
  /**
  ...
  setWorkflowRoot(root) {
    ...
    if (!this.outputChannel) {
      this.outputChannel = vscode5.window.createOutputChannel("WF: Pipeline");
    }
  ...
  async loadHistoryFromStorage() {
    ...
    try {
      const persisted = this.context.workspaceState.get("pipelineHistory");
   ...
   */
  backfillHistoryReports(persisted) {
    if (!this.workflowRoot) return false;
    const reportsDir = path6.join(this.workflowRoot, "reports");
    try {
      if (!fs4.existsSync(reportsDir)) {
        console.log("[Backfill] Reports dir not found:", reportsDir);
        return false;
      }
      const reportFiles = fs4.readdirSync(reportsDir).filter((f) => f.endsWith(".md")).map((f) => {
        const filePath = path6.join(reportsDir, f);
      ...
      const pairs2 = persisted.map((p, i) => ({
      ...
      for (let i = 0; i < pairs2.length; i++) {
        if (pairs2[i].entry.reports && pairs2[i].entry.reports.length > 0) continue;
        const runTs = pairs2[i].timestamp;
        const prevTs = i > 0 ? pairs2[i - 1].timestamp : 0;
        const matched = reportFiles.filter((r) => r.mtime <= runTs && r.mtime > prevTs);
        console.log(`[Backfill] Run #${pairs2[i].entry.runNumber}: runTs=${runTs}, prevTs=${prevTs}, matched=${matched.length}`);
        if (matched.length > 0) {
          pairs2[i].entry.reports = matched.map((r) => ({ id: r.id, path: r.path }));
          updated = true;
        }
      }
   ...
   * Returns true if any entries were updated.
   */
  backfillHistoryLogFiles(persisted) {
    if (!this.workflowRoot) return false;
    const logsDir = path6.join(this.workflowRoot, "logs");
    try {
      if (!fs4.existsSync(logsDir)) {
        console.log("[Backfill] Logs dir not found:", logsDir);
        return false;
      }
      const logFiles = fs4.readdirSync(logsDir).filter((f) => f.endsWith(".log")).map((f) => {
        const filePath = path6.join(logsDir, f);
        const stat = fs4.statSync(filePath);
  ...
  async saveHistoryToStorage() {
    if (!this.context) return;
    try {
      const persisted = this.runHistory.map((item) => ({
        runNumber: item.runNumber,
  ...
  setupPipelineListeners() {
    ...
    this.pipelineService.onStateChange((state) => {
      ...
      const isCompleted = state === "completed" /* Completed */ || state === "error" /* Error */;
      const isManuallyStopped = state === "idle" /* Idle */ && previousState === "running" /* Running */;
      if (isCompleted || isManuallyStopped) {
        ...
        const result = isManuallyStopped ? "stopped" : state === "completed" /* Completed */ ? "success" : "error";
        this.runCounter++;
        ...
        this.currentRunLogFile = void 0;
        this.currentRunPlanId = void 0;
  ...
  parseLogLine(line) {
    ...
    if (gotoMatch) {
      ...
      const elapsed = gotoNewMatch ? void 0 : gotoMatch[2];
      let gotoTarget;
      ...
      if (this.currentStage) {
        const statusChange = this.ticketStatusHistory.length > 0 ? this.ticketStatusHistory.join(" \u2192 ") : gotoTarget ? `\u2192 ${gotoTarget}` : void 0;
      ...
      this.currentStageReport = void 0;
    ...
    if (createReportMatch) {
      ...
      const reportInfo = { id: reportId, path: reportPath };
  ...
  parseLogLineLegacy(line) {
    ...
    if (line.includes("[GOTO]")) {
      ...
      if (gotoMatch) {
        if (this.currentStage) {
          const statusChange = this.ticketStatusHistory.length > 0 ? this.ticketStatusHistory.join(" \u2192 ") : void 0;
          const occ = this.stageOccurrences.get(this.currentStage) ?? 0;
          this.completedStages.push({
            stage: this.currentStage,
            elapsed: this.elapsed,
    ...
    if (!changed && this.currentStage && line.length > 0 && !line.startsWith("[PIPELINE]") && !line.startsWith("[ERROR]")) {
      if (this.currentOutputLines.length < 100) {
  ...
  getChildren(element) {
    ...
    if (element.itemType === "statistics") {
      ...
      const addStat = (label, value, icon, id) => {
        ...
        item.iconPath = new vscode5.ThemeIcon(icon);
        items.push(item);
      };
    ...
    if (element.itemType === "history-item") {
      ...
      if (reports.length === 0) {
        ...
        noReportsItem.iconPath = new vscode5.ThemeIcon("info");
   ...
   * @param planId - Optional plan ID to run pipeline for a specific plan
   */
  async startPipeline(planId) {
    if (!this.pipelineService) {
      vscode5.window.showErrorMessage(t("Pipeline service not available"));
      return;
    }
    this.completedStages = [];
    this.currentStage = void 0;
    ...
    this.currentOutputLines = [];
    this.currentRunReports = [];
    this.stageOccurrences = /* @__PURE__ */ new Map();
    this.currentRunLogFile = void 0;
    ...
    try {
      ...
      vscode5.window.showInformationMessage(t("Pipeline started"));
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unknown error";
      vscode5.window.showErrorMessage(t("Failed to start pipeline: {0}", message));
    }
  }
  /**
  ...
  stopPipeline() {
    ...
    vscode5.window.showInformationMessage(t("Pipeline stopped"));
  }
  /**
   * Show output channel
   */
  ...
  scanForNewReports() {
    ...
    const reportsDir = path6.join(this.workflowRoot, "reports");
    try {
      if (!fs4.existsSync(reportsDir)) return [];
      const files = fs4.readdirSync(reportsDir);
      const reports = [];
      for (const file of files) {
        if (!file.endsWith(".md")) continue;
        const filePath = path6.join(reportsDir, file);
        const stat = fs4.statSync(filePath);
   ...
   */
  scanForLogFile() {
    if (!this.workflowRoot || this.runStartTime === 0) return void 0;
    const logsDir = path6.join(this.workflowRoot, "logs");
    try {
      if (!fs4.existsSync(logsDir)) return void 0;
      const files = fs4.readdirSync(logsDir);
      for (const file of files) {
  ...
  clearHistory() {
    ...
    this.ticketStatusHistory = [];
    this.refresh();
    vscode5.window.showInformationMessage(t("Pipeline history cleared"));
  }
  /**
   * Dispose resources
   */
...
var DependencyService = class {
   ...
   */
  dfsVisit(id, colors, cycles, foundCycles, path19) {
    ...
    for (const depId of ticket.dependencies) {
      ...
      if (depColor === 1 /* Gray */) {
        ...
        if (cycleStart !== -1) {
          const cycle = path19.slice(cycleStart);
        ...
        this.dfsVisit(depId, colors, cycles, foundCycles, path19);
      }
    }
    colors.set(id, 2 /* Black */);
    path19.pop();
...
var ValidationService = class {
  ...
  formatAjvError(error) {
    ...
    switch (keyword) {
      case "required":
        return t('Missing required field "{0}"', params.missingProperty);
      case "type":
        return t('Field "{0}" must be of type {1}', field, params.type);
      case "pattern":
        return t('Field "{0}" does not match required pattern', field);
      case "enum":
        return t('Field "{0}" must be one of: {1}', field, params.allowedValues?.join(", "));
      case "minimum":
        return t('Field "{0}" must be >= {1}', field, params.limit);
      case "maximum":
        return t('Field "{0}" must be <= {1}', field, params.limit);
      case "minLength":
...
var DiagnosticProvider = class {
  ...
  registerFileWatchers() {
    ...
    const ticketPattern = new vscode7.RelativePattern(
      path7.join(this.workflowRoot, ".workflow", "tickets"),
      "**/*.md"
    );
  ...
  isWorkflowFile(uri) {
    ...
    return fsPath.includes(path7.join(".workflow", "tickets")) || fsPath.endsWith(path7.join(".workflow", "config", "pipeline.yaml")) || fsPath.endsWith(path7.join(".workflow", "config", "config.yaml"));
  }
  /**
  ...
  validateDocument(doc) {
    ...
    if (fsPath.endsWith(".md") && fsPath.includes(path7.join(".workflow", "tickets"))) {
      this.validateTicketDocument(uri, doc.getText());
    } else if (fsPath.endsWith(path7.join(".workflow", "config", "pipeline.yaml"))) {
      this.validatePipelineDocument(uri, doc.getText());
    } else if (fsPath.endsWith(path7.join(".workflow", "config", "config.yaml"))) {
  ...
  validateFileFromDisk(uri) {
    try {
      const content = fs5.readFileSync(uri.fsPath, "utf-8");
      const fsPath = uri.fsPath;
      if (fsPath.endsWith(".md") && fsPath.includes(path7.join(".workflow", "tickets"))) {
    ...
    try {
      ...
      const diagnostics = [
        new vscode7.Diagnostic(
          ...
          t("Failed to parse ticket frontmatter: {0}", error.message),
  ...
  validatePipelineDocument(uri, content) {
    try {
      ...
      const diagnostics = [
        new vscode7.Diagnostic(
          new vscode7.Range(0, 0, 0, 0),
          t("Failed to parse pipeline YAML: {0}", error.message),
          vscode7.DiagnosticSeverity.Error
        )
      ];
      diagnostics[0].source = "workflow-ai";
  ...
  validateConfigDocument(uri, content) {
    try {
      ...
      const diagnostics = [
        new vscode7.Diagnostic(
          new vscode7.Range(0, 0, 0, 0),
          t("Failed to parse config YAML: {0}", error.message),
          vscode7.DiagnosticSeverity.Error
        )
      ];
      diagnostics[0].source = "workflow-ai";
      ...
      if (inContextFiles) {
        ...
        if (pathMatch) {
          ...
          if (path8.isAbsolute(filePath)) {
            ...
            targetUri = vscode8.Uri.file(path8.join(this.workflowRoot, filePath));
          }
          if (fs6.existsSync(targetUri.fsPath)) {
...
var PipelineCodeLensProvider = class {
  ...
  provideCodeLenses(document) {
    ...
    const fileName = path9.basename(document.fileName);
...
var ConfigCodeLensProvider = class {
  ...
  provideCodeLenses(document) {
    ...
    const fileName = path10.basename(document.fileName);
...
var PlanCodeLensProvider = class {
  ...
  provideCodeLenses(document) {
    ...
    const isRootPlan = !normalizedPath.includes("/.workflow/") && /\/plans\/[^/]+\.md$/.test(normalizedPath);
    if (isWorkflowPlan) {
      const fileNameOnly = path10.basename(fileName);
...
var WorkflowCodeLensProvider = class {
  ...
  provideCodeLenses(document) {
    ...
    if ((normalizedPath.includes("/.workflow/plans/") || !normalizedPath.includes("/.workflow/") && /\/plans\/[^/]+\.md$/.test(normalizedPath)) && normalizedPath.endsWith(".md")) {
...
${t("Status")}: ${STATUS_ICONS2[ticket.status]} ${ticket.status}
...
${t("Located at")}: \`.workflow/src/skills/${skillId}/SKILL.md\``
  ...
  createStageDocumentation(stageId, stageConfig) {
    const description = typeof stageConfig === "string" ? stageConfig : stageConfig.description || t("No description");
    const doc = new vscode11.MarkdownString(`**${t("Stage")}: ${stageId}**
...
- \`${trigger}\` \u2192 \`${targetStage}\``);
  ...
  createAgentDocumentation(agentId, agentConfig) {
    ...
    const doc = new vscode11.MarkdownString(`**${t("Agent")}: ${agentId}**
...
`);
    markdown.appendMarkdown(
      `**${t("Status")}:** ${statusIcon} \`${ticket.status}\`  | **${t("Priority")}:** ${priorityIcon} \`${ticket.priority}\`  | **${t("Type")}:** ${typeIcon} \`${ticket.type}\`  | **${t("Complexity")}:** ${complexityIcon} \`${ticket.complexity}\`
...
var NotificationsManager = class {
  ...
  showTicketCompletedNotification(ticketId, _ticket) {
    ...
    vscode14.window.showInformationMessage(message, t("Open")).then((selection) => {
      if (selection === "Open") {
  ...
  showTicketBlockedNotification(ticketId, _ticket) {
    ...
    vscode14.window.showWarningMessage(message, t("Details")).then((selection) => {
  ...
  showPipelineErrorNotification() {
    ...
    vscode14.window.showErrorMessage(message, t("View Log")).then((selection) => {
  ...
  showPipelineCompletedNotification() {
    ...
    vscode14.window.showInformationMessage(message, t("Report")).then((selection) => {
      if (selection === "Report") {
  ...
  async openTicketFile(ticketId, status) {
    if (!this.workflowRoot) {
      vscode14.window.showErrorMessage(t("Workflow root not available"));
      return;
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Unknown error";
      vscode14.window.showErrorMessage(t("Failed to open ticket: {0}", message));
    }
  }
  /**
  ...
  async openLatestReport() {
    ...
    const reports = this.store.getReports();
    if (reports.length === 0) {
      vscode14.window.showInformationMessage(t("No reports available"));
      return;
    }
    ...
    if (!latestReport) {
      vscode14.window.showInformationMessage(t("No reports with date available"));
      return;
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Unknown error";
      vscode14.window.showErrorMessage(t("Failed to open report: {0}", message));
    }
  }
  /**
   * Set workflow root directory
...
var TicketService = class {
   ...
   */
  getByPlan(planId) {
    return this.getAll().filter((t2) => t2.parent_plan === planId);
  }
  /**
   * Get tickets filtered by type
   */
  getByType(type2) {
    return this.getAll().filter((t2) => t2.type === type2);
  }
  // ==================== State Machine ====================
  /**
  ...
  async create(type2, title, fields) {
    ...
    const templatePath = path12.join(this.workflowRoot, "templates", "ticket-template.md");
    let templateContent;
    try {
    ...
    const backlogDir = path12.join(this.workflowRoot, "tickets", "backlog");
    const filePath = path12.join(backlogDir, `${id}.md`);
    await fs7.mkdir(backlogDir, { recursive: true });
  ...
  async moveTicketDirect(id, currentStatus, targetStatus) {
    const sourcePath = path12.join(this.workflowRoot, "tickets", currentStatus, `${id}.md`);
    const targetDir = path12.join(this.workflowRoot, "tickets", targetStatus);
    const targetPath = path12.join(targetDir, `${id}.md`);
    let content;
    try {
      content = await fs7.readFile(sourcePath, "utf-8");
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
    ...
    await this.withOwnWrite(async () => {
      await fs7.writeFile(targetPath, updatedContent, "utf-8");
    });
    await this.withOwnWrite(async () => {
      await fs7.unlink(sourcePath);
    });
  }
  ...
  async update(id, fields) {
    ...
    await this.withOwnWrite(async () => {
      await fs7.writeFile(filePath, updatedContent, "utf-8");
    });
    const updatedTicket = {
      ...ticket,
      ...fields,
...
var FileWatcherService = class {
  ...
  createWatcher() {
    ...
    this.fileWatcher = vscode15.workspace.createFileSystemWatcher(
      pattern,
      false,
      // ignoreCreateEvents
      false,
   ...
   */
  classifyChange(uri) {
    const relativePath = path13.relative(this.workflowRoot, uri.fsPath);
...
async function executeNewTicket(ticketService) {
  const type2 = await vscode16.window.showQuickPick(
    [
  ...
  try {
    ...
    vscode16.window.showInformationMessage(t("Created ticket {0}: {1}", ticket.id, ticket.title));
    const workflowRoot = ticketService.getWorkflowRoot();
    if (workflowRoot) {
      const ticketPath = vscode16.Uri.file(
        path14.join(workflowRoot, "tickets", "backlog", `${ticket.id}.md`)
      );
      await vscode16.commands.executeCommand("vscode.open", ticketPath);
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    vscode16.window.showErrorMessage(t("Failed to create ticket: {0}", message));
  }
}

// src/commands/new-plan.ts
var vscode17 = __toESM(require("vscode"));
var path15 = __toESM(require("path"));
async function executeNewPlan(planService) {
  ...
  try {
    ...
    vscode17.window.showInformationMessage(t("Created plan {0}: {1}", plan.id, plan.title));
    const workflowRoot = planService.getWorkflowRoot();
    const planPath = vscode17.Uri.file(
      path15.join(workflowRoot, "plans", "current", `${plan.id}.md`)
    );
    await vscode17.commands.executeCommand("vscode.open", planPath);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    vscode17.window.showErrorMessage(t("Failed to create plan: {0}", message));
  }
}
...
var PlanService = class {
   ...
   */
  async create(title, fields) {
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const nextId = await this.generateNextPlanId();
    const templatePath = path16.join(this.workflowRoot, "templates", "plan-template.md");
    let templateContent;
    try {
    ...
    const plansDir = path16.join(this.workflowRoot, "plans", "current");
    const filePath = path16.join(plansDir, nextId + ".md");
   ...
   */
  async generateNextPlanId() {
    const allPlans = this.getAll();
    if (allPlans.length === 0) {
      return "PLAN-001";
    }
    const planNumbers = allPlans.map((plan) => plan.id).filter((id) => id.startsWith("PLAN-")).map((id) => parseInt(id.replace("PLAN-", ""), 10)).filter((num) => !isNaN(num));
   ...
   */
  generatePlanFileContent(plan, body) {
  ...
  getPlanProgress(planId) {
    ...
    const done = tickets.filter(
      (ticket) => ticket.status === "done" /* Done */
    ).length;
  ...
  async archive(id) {
    ...
    const currentDir = path16.join(this.workflowRoot, "plans", "current");
    const archiveDir = path16.join(this.workflowRoot, "plans", "archive");
    const currentPath = path16.join(currentDir, id + ".md");
    const archivePath = path16.join(archiveDir, id + ".md");
    let body = "";
    try {
      const content = await fs8.readFile(currentPath, "utf-8");
      const { body: parsedBody } = parse(content);
      body = parsedBody;
    } catch (error) {
      body = "";
    }
    ...
    await fs8.writeFile(archivePath, newContent, "utf-8");
    try {
      await fs8.unlink(currentPath);
    } catch (error) {
...
async function executeShowDependencies(store, dependencyService, ticketId) {
  ...
  if (!ticketId) {
    ...
    if (tickets.length === 0) {
      vscode18.window.showInformationMessage(t("No tickets available"));
      return;
    }
    const selected2 = await vscode18.window.showQuickPick(
      tickets.map((ticket2) => ({
        label: ticket2.id,
        description: ticket2.title,
        detail: t("Status: {0}", ticket2.status)
      })),
      {
  ...
  if (!ticket) {
    vscode18.window.showErrorMessage(t("Ticket {0} not found", ticketId));
    return;
  }
...
async function executeShowStatistics(store) {
  const tickets = store.getTickets();
  if (tickets.length === 0) {
    vscode19.window.showInformationMessage(t("No tickets to analyze"));
    return;
  }
  ...
  if (copyAction === "Copy") {
    ...
    vscode19.window.showInformationMessage(t("Statistics copied to clipboard"));
  }
}
...
async function executeOpenPipelineConfig(workflowRoot) {
  if (!workflowRoot) {
    vscode20.window.showErrorMessage(t("Workflow not found"));
    return;
  }
  const configPath = path17.join(workflowRoot, "config", "pipeline.yaml");
  const uri = vscode20.Uri.file(configPath);
  try {
    await vscode20.commands.executeCommand("vscode.open", uri);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    vscode20.window.showErrorMessage(t("Failed to open pipeline config: {0}", message));
  }
}
async function executeOpenConfig(workflowRoot) {
  ...
  const configPath = path17.join(workflowRoot, "config", "config.yaml");
  const uri = vscode20.Uri.file(configPath);
  try {
    await vscode20.commands.executeCommand("vscode.open", uri);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    vscode20.window.showErrorMessage(t("Failed to open config: {0}", message));
  }
}
...
async function executeRefreshAll(workflowRoot, store, refreshCallbacks) {
  ...
  vscode20.window.showInformationMessage(t("Workflow data refreshed"));
}
async function executeCopyTicketId(ticketId) {
  ...
  if (!ticketId) {
    vscode20.window.showErrorMessage(t("No ticket ID provided or found"));
    return;
  }
  await vscode20.env.clipboard.writeText(ticketId);
  vscode20.window.showInformationMessage(t("Copied {0} to clipboard", ticketId));
}
async function executeFilterTicketsByPlan(store, ticketsProvider, kanbanProviders) {
  const plans = store.getPlans();
  if (plans.length === 0) {
    vscode20.window.showInformationMessage(t("No plans available"));
    return;
  }
...
async function executeClearTicketFilter(ticketsProvider, kanbanProviders) {
  ...
  vscode20.window.showInformationMessage(t("Ticket filter cleared"));
}
...
var ErrorHandler = class {
  ...
  sessionStart;
  constructor() {
    this.outputChannel = vscode21.window.createOutputChannel("Workflow AI");
  ...
  log(level, message, data) {
    if (this.shouldLog(level)) {
      ...
      this.outputChannel.appendLine(`[${timestamp2}] [${level}] ${message}${dataStr}`);
    }
  }
  /**
  ...
  handleError(error, context, errorContext) {
    ...
    this.log("ERROR" /* ERROR */, `Error in ${context}: ${errorMessage}`, {
      ...errorContext?.data,
...
${stack}`);
    ...
    if (shouldShowMessage) {
      ...
      vscode21.window.showErrorMessage(userMessage);
    }
  }
  /**
   * Get a user-friendly error message.
   */
  getUserFriendlyMessage(error, context, customMessage) {
    if (customMessage) {
   ...
   */
  logCommandEnd(command, duration) {
    const durationStr = duration !== void 0 ? ` in ${duration}ms` : "";
    this.info(`Command completed: ${command}${durationStr}`);
  }
  /**
   * Log command execution error.
   */
  logCommandError(command, error, duration) {
    const durationStr = duration !== void 0 ? ` after ${duration}ms` : "";
    this.handleError(error, `Command: ${command}${durationStr}`, {
      showMessage: false
      // Command errors are handled separately
    });
  }
  /**
   * Log CLI execution result.
   */
  logCliResult(command, stdout, stderr, exitCode) {
   ...
   */
  logValidationResult(type2, isValid, errors) {
    if (isValid) {
      this.debug(`Validation passed: ${type2}`);
    } else {
      this.warn(`Validation failed: ${type2}`, { errors });
    }
  }
  /**
   * Log performance metrics.
   */
  logPerformance(operation, durationMs, threshold = 1e3) {
...
async function checkCliInstalled(errorHandler) {
  try {
    ...
    const config = vscode22.workspace.getConfiguration("workflow");
...
function checkWorkflowDir(errorHandler) {
  ...
  const workflowDir = path18.join(workspaceRoot, ".workflow");
  const configPath = path18.join(workflowDir, "config", "config.yaml");
  const pipelinePath = path18.join(workflowDir, "config", "pipeline.yaml");
  try {
    const dirExists = fs9.existsSync(workflowDir);
    const configExists = fs9.existsSync(configPath);
    const pipelineExists = fs9.existsSync(pipelinePath);
...
async function setContextKey(key, value) {
  await vscode22.commands.executeCommand("setContext", key, value);
}
  ...
  await vscode22.window.withProgress(
    ...
    async (progress) => {
      ...
      try {
        ...
        await updateContextKeys(void 0, errorHandler);
        vscode22.window.showInformationMessage(t("workflow-ai CLI installed successfully!"));
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to install workflow-ai CLI: {0}", message));
        }
      }
    }
  );
    ...
    async (progress) => {
      ...
      try {
        ...
        if (!workspaceRoot) {
          ...
          const result = await vscode22.window.showWarningMessage(
            t("Please open a folder first to initialize Workflow."),
            open
          );
          if (result === open) {
            await vscode22.commands.executeCommand("vscode.openFolder");
          }
          return;
        }
        try {
        ...
        progress.report({ increment: 100 });
        await updateContextKeys(void 0, errorHandler);
        vscode22.window.showInformationMessage(t("Workflow initialized successfully!"));
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to initialize workflow: {0}", message));
        }
      }
    }
  );
...
async function activate(context) {
  ...
  const registerCommandSafe = (command, callback) => {
    try {
      return vscode22.commands.registerCommand(command, callback);
    } catch (err) {
  ...
  if (workspaceRoot && checkWorkflowDir()) {
    workflowRoot = path18.join(workspaceRoot, ".workflow");
  ...
  context.subscriptions.push(
    vscode22.window.registerTreeDataProvider("workflow-sidebar.tickets", ticketsProvider),
    vscode22.window.registerTreeDataProvider("workflow-sidebar.plans", plansProvider),
    vscode22.window.registerTreeDataProvider("workflow-sidebar.reports", reportsProvider),
    vscode22.window.registerTreeDataProvider("workflow-sidebar.skills", skillsProvider),
    vscode22.window.registerTreeDataProvider("workflow-sidebar.logs", logsProvider),
    vscode22.window.registerTreeDataProvider("workflow-sidebar.pipeline", pipelineProvider),
    statusBar,
    skillsProvider,
    logsProvider
  );
  const backlogTreeView = vscode22.window.createTreeView("wf-kanban-backlog", { treeDataProvider: kanbanProviders.backlog });
  const readyTreeView = vscode22.window.createTreeView("wf-kanban-ready", { treeDataProvider: kanbanProviders.ready });
  const inProgressTreeView = vscode22.window.createTreeView("wf-kanban-in-progress", { treeDataProvider: kanbanProviders.inProgress });
  const blockedTreeView = vscode22.window.createTreeView("wf-kanban-blocked", { treeDataProvider: kanbanProviders.blocked });
  const reviewTreeView = vscode22.window.createTreeView("wf-kanban-review", { treeDataProvider: kanbanProviders.review });
  const doneTreeView = vscode22.window.createTreeView("wf-kanban-done", { treeDataProvider: kanbanProviders.done });
  context.subscriptions.push(
    backlogTreeView,
    readyTreeView,
    inProgressTreeView,
  ...
  const updateKanbanTitles = () => {
    ...
    const plan = filterPlan ? store.getPlanById(filterPlan) : void 0;
    const planTitle = plan ? `: ${plan.title}` : "";
    const filterPrefix = filterPlan ? `\u{1F50D} ${filterPlan}${planTitle} \u2014 ` : "";
  ...
  const documentLinkDisposable = vscode22.languages.registerDocumentLinkProvider(
    [
  ...
  if (workflowRoot) {
    ...
    const codeLensDisposable = vscode22.languages.registerCodeLensProvider(
      [
        { scheme: "file", pattern: "**/.workflow/tickets/**/*.md" },
  ...
  if (workflowRoot) {
    ...
    const completionDisposable = vscode22.languages.registerCompletionItemProvider(
      [
        { scheme: "file", pattern: "**/.workflow/tickets/**/*.md" },
        { scheme: "file", pattern: "**/.workflow/config/pipeline.yaml" }
      ],
  ...
  if (workflowRoot) {
    ...
    const hoverDisposable = vscode22.languages.registerHoverProvider(
      [
        { scheme: "file", pattern: "**/.workflow/tickets/**/*.md" },
        { scheme: "file", pattern: "**/.workflow/config/pipeline.yaml" },
        { scheme: "file", pattern: "**/.workflow/plans/**/*.md" },
  ...
  registerCommandSafe(
    ...
    async (stageId) => {
      if (!workflowRoot) {
        vscode22.window.showErrorMessage(t("Workflow root not available"));
        return;
      }
      const pipelinePath = path18.join(workflowRoot, "config", "pipeline.yaml");
      try {
        const doc = await vscode22.workspace.openTextDocument(pipelinePath);
        ...
        if (match) {
          const textBeforeMatch = content.substring(0, match.index);
          const lineNumber = (textBeforeMatch.match(/\n/g) || []).length;
          const position = new vscode22.Position(lineNumber, 0);
          editor.revealRange(
            new vscode22.Range(position, position),
            vscode22.TextEditorRevealType.InCenter
          );
          editor.selection = new vscode22.Selection(position, position);
        }
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to focus on stage: {0}", message));
        }
      }
    }
  );
  const openTicketCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      if (!ticketId) {
        ...
        if (editor) {
          ticketId = path18.basename(editor.document.fileName, ".md");
        }
      }
      if (!ticketId || !workflowRoot) {
        vscode22.window.showErrorMessage(t("No ticket ID provided or workflow not available"));
        return;
      }
      const ticket = store.getTicketById(ticketId);
      if (!ticket) {
        vscode22.window.showErrorMessage(t("Ticket {0} not found", ticketId));
        return;
      }
      const ticketPath = path18.join(
        workflowRoot,
        "tickets",
        ticket.status,
        `${ticketId}.md`
      ...
      try {
        await vscode22.commands.executeCommand("vscode.open", vscode22.Uri.file(ticketPath));
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to open ticket: {0}", message));
        }
      }
    }
  );
  const moveTicketCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      if (!ticketService || !ticketId) {
        vscode22.window.showErrorMessage(t("Ticket service not available or no ticket ID provided"));
        return;
      }
      ...
      if (validTransitions.length === 0) {
        vscode22.window.showInformationMessage(t("No valid transitions from {0}", ticket.status));
        return;
      }
      const targetStatus = await vscode22.window.showQuickPick(
      ...
      try {
        ...
        vscode22.window.showInformationMessage(t("Moved {0} to {1}", ticketId, targetStatus.label));
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to move ticket: {0}", message));
        }
      }
    }
  );
  const moveTicketFromMenuCmd = registerCommandSafe(
    ...
    async (arg) => {
      const ticketId = resolveTicketId(arg);
      await vscode22.commands.executeCommand("workflow.moveTicket", ticketId);
    }
  );
  const moveTicketNextCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      try {
        ...
        vscode22.window.showInformationMessage(t("Moved {0} to {1}", ticketId, nextStatus));
      } catch (error) {
        if (errorHandler) {
    ...
    async (arg) => {
      try {
        ...
        if (!dependencyService || !ticketId) {
          vscode22.window.showErrorMessage(t("Dependency service not available or no ticket ID provided"));
          return;
        }
        const ticket = store.getTicketById(ticketId);
        if (!ticket) {
...
**Blocks:**
${blocksList}`;
        await vscode22.window.showInformationMessage(info, { modal: false });
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to show dependencies: {0}", message));
        }
      }
    }
  );
  const showTicketDependenciesCmd = registerCommandSafe(
    ...
    async (arg) => {
      const ticketId = resolveTicketId(arg);
      await vscode22.commands.executeCommand("workflow.showDependencies", ticketId);
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to refresh tickets: {0}", message));
        }
      }
    }
  );
  ...
  const sortKanbanByPriorityCmd = registerCommandSafe(
    ...
    async () => {
      try {
        ...
        vscode22.window.showInformationMessage(t("Kanban boards sorted by priority"));
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to sort kanban: {0}", message));
        }
      }
    }
  );
  const sortKanbanByIdCmd = registerCommandSafe(
    ...
    async () => {
      try {
        ...
        vscode22.window.showInformationMessage(t("Kanban boards sorted by ID"));
      } catch (error) {
        if (errorHandler) {
  ...
  const sortKanbanByTitleCmd = registerCommandSafe(
    ...
    async () => {
      try {
        ...
        vscode22.window.showInformationMessage(t("Kanban boards sorted by title"));
      } catch (error) {
        if (errorHandler) {
  ...
  const sortKanbanByDateCmd = registerCommandSafe(
    ...
    async () => {
      try {
        ...
        vscode22.window.showInformationMessage(t("Kanban boards sorted by date"));
      } catch (error) {
        if (errorHandler) {
  ...
  const toggleSortDirectionCmd = registerCommandSafe(
    ...
    async () => {
      try {
        ...
        vscode22.window.showInformationMessage(t("Sort direction set to {0}", direction));
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to toggle sort direction: {0}", message));
        }
      }
    }
  );
  ...
  const sortTicketsByPriorityCmd = registerCommandSafe(
    ...
    async () => {
      try {
        ...
        vscode22.window.showInformationMessage(t("Tickets sidebar sorted by priority"));
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to sort tickets: {0}", message));
        }
      }
    }
  );
  const sortTicketsByIdCmd = registerCommandSafe(
    ...
    async () => {
      try {
        ...
        vscode22.window.showInformationMessage(t("Tickets sidebar sorted by ID"));
      } catch (error) {
        if (errorHandler) {
  ...
  const sortTicketsByTitleCmd = registerCommandSafe(
    ...
    async () => {
      try {
        ...
        vscode22.window.showInformationMessage(t("Tickets sidebar sorted by title"));
      } catch (error) {
        if (errorHandler) {
  ...
  const sortTicketsByDateCmd = registerCommandSafe(
    ...
    async () => {
      try {
        ...
        vscode22.window.showInformationMessage(t("Tickets sidebar sorted by date"));
      } catch (error) {
        if (errorHandler) {
  ...
  const gotoReviewSectionCmd = registerCommandSafe(
    ...
    async () => {
      try {
        const editor = vscode22.window.activeTextEditor;
        if (!editor) {
          vscode22.window.showErrorMessage(t("No active editor"));
          return;
        }
        ...
        if (!reviewMatch || reviewMatch.index === void 0) {
          vscode22.window.showInformationMessage(t("No Review section found in this document"));
          return;
        }
        const position = document.positionAt(reviewMatch.index);
        await editor.revealRange(new vscode22.Range(position, position), vscode22.TextEditorRevealType.InCenter);
        editor.selection = new vscode22.Selection(position, position);
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to go to review section: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to start pipeline: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to stop pipeline: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to show pipeline output: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to clear pipeline history: {0}", message));
        }
      }
    }
  );
  const statusBarClickCmd = registerCommandSafe(
    ...
    async () => {
      try {
        await vscode22.commands.executeCommand("workbench.action.quickOpen", ">WF:");
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to open command palette: {0}", message));
        }
      }
    }
  );
  const newTicketCmd = registerCommandSafe(
    ...
    async () => {
      try {
        if (!ticketService) {
          vscode22.window.showErrorMessage(t("Ticket service not available"));
          return;
        }
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to create new ticket: {0}", message));
        }
      }
    }
  );
  const newPlanCmd = registerCommandSafe(
    ...
    async () => {
      try {
        if (!planService) {
          vscode22.window.showErrorMessage(t("Workflow not found"));
          return;
        }
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to create plan: {0}", message));
        }
      }
    }
  );
    ...
    async (arg) => {
      try {
        ...
        if (!planId) {
          ...
          if (plans.length === 0) {
            vscode22.window.showInformationMessage(t("No current plans available"));
            return;
          }
          plans.sort((a, b) => a.id.localeCompare(b.id));
        ...
        const planPath = path18.join(workflowRoot, "plans", "current", `${planId}.md`);
        let actualPlanPath = planPath;
        if (!fs9.existsSync(planPath)) {
          actualPlanPath = path18.join(workflowRoot, "plans", "archive", `${planId}.md`);
        }
        if (!fs9.existsSync(actualPlanPath)) {
          vscode22.window.showErrorMessage(t("Plan {0} not found", planId));
          return;
        }
        const pipelinePath = path18.join(workflowRoot, "config", "pipeline.yaml");
        if (!fs9.existsSync(pipelinePath)) {
          vscode22.window.showErrorMessage(t("Pipeline config not found: {0}", pipelinePath));
          return;
        }
        const pipelineContent = fs9.readFileSync(pipelinePath, "utf-8");
        const pipelineData = load(pipelineContent);
        const defaultAgentId = pipelineData?.pipeline?.default_agent;
        const agent = defaultAgentId ? pipelineData?.pipeline?.agents?.[defaultAgentId] : void 0;
        if (!agent) {
          vscode22.window.showErrorMessage(t("Default agent not configured in pipeline.yaml"));
          return;
        }
        ...
        const terminalEnv = { CLAUDECODE: null };
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to decompose plan: {0}", message));
        }
      }
    }
  );
  const createPlanFromFileCmd = registerCommandSafe(
    ...
    async (arg) => {
      try {
        let documentUri;
        if (arg instanceof vscode22.Uri) {
          documentUri = arg;
        } else if (arg && typeof arg === "object") {
          const item = arg;
          if (item.uri && item.uri instanceof vscode22.Uri) {
            documentUri = item.uri;
          }
        }
        if (!documentUri) {
          vscode22.window.showErrorMessage(t("Document not available"));
          return;
        }
        if (!workflowRoot) {
          vscode22.window.showErrorMessage(t("Workflow root not available"));
        ...
        const sourceFileName = path18.basename(sourcePath);
        const pipelinePath = path18.join(workflowRoot, "config", "pipeline.yaml");
        if (!fs9.existsSync(pipelinePath)) {
          vscode22.window.showErrorMessage(t("Pipeline config not found: {0}", pipelinePath));
          return;
        ...
        vscode22.window.withProgress(
          ...
          () => new Promise((resolve) => {
            const disposable = vscode22.window.onDidCloseTerminal((closedTerminal) => {
  ...
  const runPipelineForPlanCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      if (!planId || !workflowRoot) {
        vscode22.window.showErrorMessage(t("Plan ID not available or workflow not found"));
        return;
      }
      const planPath = path18.join(workflowRoot, "plans", "current", `${planId}.md`);
      if (!fs9.existsSync(planPath)) {
        vscode22.window.showErrorMessage(t("Plan {0} not found in current plans", planId));
        return;
      }
      try {
        await pipelineProvider.startPipeline(planId);
        vscode22.window.showInformationMessage(t("Pipeline started for plan {0}", planId));
      } catch (error) {
        const message = error instanceof Error ? error.message : "Unknown error";
        if (message.includes("cancel") || message.includes("abort")) {
          vscode22.window.showInformationMessage(t("Pipeline cancelled"));
        } else {
          if (errorHandler) {
            ...
            vscode22.window.showErrorMessage(t("Pipeline failed for plan {0}: {1}", planId, message));
          }
        }
      }
    }
  ...
  const archivePlanCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      const sourcePath = path18.join(workflowRoot, "plans", "current", `${planId}.md`);
      const targetPath = path18.join(workflowRoot, "plans", "archive", `${planId}.md`);
      if (!fs9.existsSync(sourcePath)) {
        vscode22.window.showErrorMessage(t("Plan {0} not found in current plans", planId));
        return;
      }
      try {
        const archiveDir = path18.join(workflowRoot, "plans", "archive");
        if (!fs9.existsSync(archiveDir)) {
          fs9.mkdirSync(archiveDir, { recursive: true });
        }
        fs9.renameSync(sourcePath, targetPath);
        await store.refresh(workflowRoot);
        vscode22.window.showInformationMessage(t("Plan {0} archived", planId));
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to archive plan {0}: {1}", planId, message));
        }
      }
    }
  );
  const unarchivePlanCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      const sourcePath = path18.join(workflowRoot, "plans", "archive", `${planId}.md`);
      const targetPath = path18.join(workflowRoot, "plans", "current", `${planId}.md`);
      if (!fs9.existsSync(sourcePath)) {
        vscode22.window.showErrorMessage(t("Plan {0} not found in archive", planId));
        return;
      }
      try {
        const currentDir = path18.join(workflowRoot, "plans", "current");
        if (!fs9.existsSync(currentDir)) {
          fs9.mkdirSync(currentDir, { recursive: true });
        }
        fs9.renameSync(sourcePath, targetPath);
        await store.refresh(workflowRoot);
        vscode22.window.showInformationMessage(t("Plan {0} unarchived", planId));
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to unarchive plan {0}: {1}", planId, message));
        }
      }
    }
  );
    ...
    async (arg) => {
      try {
        ...
        if (!dependencyService) {
          vscode22.window.showErrorMessage(t("Dependency service not available"));
          return;
        }
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to show statistics: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to open pipeline config: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to open config: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to focus tickets view: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to focus kanban: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to refresh all: {0}", message));
        }
      }
    }
  );
    ...
    async (arg) => {
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to copy ticket ID: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      ...
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to filter tickets: {0}", message));
        }
      }
    }
  );
    ...
    async () => {
      ...
      try {
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to clear filter: {0}", message));
        }
      }
    }
  );
  const openHistoryLogCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      if (!logFile) {
        vscode22.window.showErrorMessage(t("No log file provided"));
        return;
      }
      try {
        const doc = await vscode22.workspace.openTextDocument(logFile);
        ...
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to open log file: {0}", message));
        }
      }
    }
  );
  const openHistoryPlanCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      const currentPath = path18.join(workflowRoot, "plans", "current", `${planId}.md`);
      const archivePath = path18.join(workflowRoot, "plans", "archive", `${planId}.md`);
      let planPath;
      if (fs9.existsSync(currentPath)) {
        planPath = currentPath;
      } else if (fs9.existsSync(archivePath)) {
      ...
      try {
        const doc = await vscode22.workspace.openTextDocument(planPath);
        await vscode22.window.showTextDocument(doc);
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to open plan: {0}", message));
        }
      }
    }
  );
  const openStageTicketCmd = registerCommandSafe(
    ...
    async (arg) => {
      const ticketId = resolveTicketId(arg);
      if (!ticketId || !workflowRoot) {
        vscode22.window.showErrorMessage(t("Ticket ID not available or workflow not found"));
        return;
      }
      const ticket = store.getTicketById(ticketId);
      if (!ticket) {
      ...
      try {
        const doc = await vscode22.workspace.openTextDocument(ticketPath);
        await vscode22.window.showTextDocument(doc);
      } catch (error) {
        if (errorHandler) {
  ...
  const openStageLogCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      try {
        ...
        if (arg && typeof arg === "object") {
          const item = arg;
          const stageName = typeof item.stage === "string" ? item.stage : void 0;
          if (stageName) {
            ...
            if (startLine >= 0) {
              ...
              const from = new vscode22.Position(startLine, 0);
              const to = new vscode22.Position(endLine, lines[endLine].length);
              editor.selection = new vscode22.Selection(from, to);
              editor.revealRange(new vscode22.Range(from, to), vscode22.TextEditorRevealType.InCenter);
            }
          }
        }
      } catch (error) {
  ...
  const openStageReportCmd = registerCommandSafe(
    ...
    async (arg) => {
      ...
      if (!reportPath) {
        vscode22.window.showErrorMessage(t("No report path provided"));
        return;
      }
      try {
        const doc = await vscode22.workspace.openTextDocument(reportPath);
        await vscode22.window.showTextDocument(doc);
      } catch (error) {
        if (errorHandler) {
          ...
          const message = error instanceof Error ? error.message : "Unknown error";
          vscode22.window.showErrorMessage(t("Failed to open report: {0}", message));
        }
      }
    }
  );
