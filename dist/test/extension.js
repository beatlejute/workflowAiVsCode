...
async function activate(context) {
     ...
     */
    const openTicketCmd = vscode.commands.registerCommand('workflow.openTicket', async (ticketId) => {
        if (!ticketId) {
     ...
     */
    const moveTicketCmd = vscode.commands.registerCommand('workflow.moveTicket', async (ticketId) => {
     ...
     */
    const moveTicketFromMenuCmd = vscode.commands.registerCommand('workflow.moveTicketFromMenu', async (ticketId) => {
     ...
     */
    const moveTicketNextCmd = vscode.commands.registerCommand('workflow.moveTicketNext', async (ticketId) => {
        if (!ticketService || !ticketId) {
            vscode.window.showErrorMessage('Ticket service not available or no ticket ID provided');
            return;
        }
     ...
     */
    const editTicketCmd = vscode.commands.registerCommand('workflow.editTicket', async (ticketId) => {
        if (!ticketId) {
            // Try to get from active editor or selection
            const editor = vscode.window.activeTextEditor;
            if (editor) {
     ...
     */
    const showDependenciesCmd = vscode.commands.registerCommand('workflow.showDependencies', async (ticketId) => {
     ...
     */
    const showTicketDependenciesCmd = vscode.commands.registerCommand('workflow.showTicketDependencies', async (ticketId) => {
     ...
     */
    const createTicketCmd = vscode.commands.registerCommand('workflow.createTicket', async () => {
        if (!ticketService) {
     ...
     */
    const refreshTicketsCmd = vscode.commands.registerCommand('workflow.refreshTickets', async () => {
     ...
     */
    const sortKanbanByPriorityCmd = vscode.commands.registerCommand('workflow.sortKanbanByPriority', async () => {
     ...
     */
    const sortKanbanByIdCmd = vscode.commands.registerCommand('workflow.sortKanbanById', async () => {
     ...
     */
    const sortKanbanByTitleCmd = vscode.commands.registerCommand('workflow.sortKanbanByTitle', async () => {
        // Note: This would require modifying the provider to support different sort modes
        // For now, just refresh to show the default sorting
        kanbanProviders.backlog.refresh();
        kanbanProviders.ready.refresh();
     ...
     */
    const gotoReviewSectionCmd = vscode.commands.registerCommand('workflow.gotoReviewSection', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
     ...
     */
    const startPipelineCmd = vscode.commands.registerCommand('workflow.runPipeline', async () => {
        await pipelineProvider.startPipeline();
    });
    /**
     * Stop pipeline command
     */
    const stopPipelineCmd = vscode.commands.registerCommand('workflow.stopPipeline', async () => {
        pipelineProvider.stopPipeline();
    });
    /**
     * Show pipeline output command
     */
    const showPipelineOutputCmd = vscode.commands.registerCommand('workflow.showPipelineOutput', async () => {
        pipelineProvider.showOutput();
    });
    /**
     * Clear pipeline history command
     */
    const clearPipelineHistoryCmd = vscode.commands.registerCommand('workflow.clearPipelineHistory', async () => {
        pipelineProvider.clearHistory();
    });
    /**
     * Status bar click command - opens Command Palette with WF: prefix
     */
    const statusBarClickCmd = vscode.commands.registerCommand('workflow.statusBarClick', async () => {
     ...
     */
    const newTicketCmd = vscode.commands.registerCommand('workflow.newTicket', async () => {
        if (!ticketService) {
            vscode.window.showErrorMessage('Ticket service not available');
            return;
        }
     ...
     */
    const newPlanCmd = vscode.commands.registerCommand('workflow.newPlan', async () => {
     ...
     */
    const showDependenciesCmdNew = vscode.commands.registerCommand('workflow.showDependencies', async (ticketId) => {
     ...
     */
    const showStatisticsCmd = vscode.commands.registerCommand('workflow.showStatistics', async () => {
        await (0, show_statistics_1.executeShowStatistics)(store);
    });
    /**
     * workflow.openPipelineConfig command - opens pipeline.yaml
     */
    const openPipelineConfigCmd = vscode.commands.registerCommand('workflow.openPipelineConfig', async () => {
        await (0, index_1.executeOpenPipelineConfig)(workflowRoot);
    });
    /**
     * workflow.openConfig command - opens config.yaml
     */
    const openConfigCmd = vscode.commands.registerCommand('workflow.openConfig', async () => {
        await (0, index_1.executeOpenConfig)(workflowRoot);
    });
    /**
     * workflow.focusTicketsView command - focuses tickets sidebar
     */
    const focusTicketsViewCmd = vscode.commands.registerCommand('workflow.focusTicketsView', async () => {
        await (0, index_1.executeFocusTicketsView)();
    });
    /**
     * workflow.focusKanban command - focuses kanban panel
     */
    const focusKanbanCmd = vscode.commands.registerCommand('workflow.focusKanban', async () => {
        await (0, index_1.executeFocusKanban)();
    });
    /**
     * workflow.refreshAll command - refreshes all data
     */
    const refreshAllCmd = vscode.commands.registerCommand('workflow.refreshAll', async () => {
